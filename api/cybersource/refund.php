...
try {
    ...
    $response = lm_payment_service($config, $logger)->refund($input['payment_id'], $input['amount'], $currency);
    ...
    $logger->error('Error en refund', array('message' => $e->getMessage()));
    lm_json_response(array('success' => false, 'message' => 'No se pudo procesar el reembolso.'), 500);
}
